...
class CNF:
	...
	def generateAssumption(self, k):
		var = 'a' + str(k)
		self.assignKey(var, 11)
		return var
	...
	def parseOutput(self, nameFile, controllerStates, parser, print_policy = False):
		...
		if 'UNSAT' in res[0]:
			return False
		if 'INDET' in res[0] or 'UNKNOWN' in res[0]:
			return None
		if not print_policy:
			return True
	...
	def addClause(self, clause, trackdups=False):
    #print(clause)
	 	clause_list = []
		varak = self.generateAssumption(self.kk)
		clause_list.append(self.mapVariableNumber[varak])
		...
		for j in clause:
			...
			if negated:
				...
				clause_list.append(-1 * self.mapVariableNumber[var])
			else:
				#self.file_formula.write(str(self.mapVariableNumber[var]) + '\t')
				clause_list.append(self.mapVariableNumber[var])
		#self.file_formula.write('0\n')
		clause_list.sort()
		if trackdups:
			self.total += 1
			if self.added_clauses.has_key(str(clause_list)):
				self.dups += 1
		self.ms.add_clause(clause_list)
		#self.added_clauses[str(clause_list)] = True

	def addCls(self, clause, trackdups=False):
    #print(clause)
	 	clause_list = []
		self.number_clauses += 1
		for j in clause:
	...
	def generatePreconditions(self, task, controllerStates, debug = False):
		...
		for n in controllerStates[len(controllerStates)-2:(len(controllerStates)-1 if len(controllerStates) > 2 else len(controllerStates))]:
			for a in actions:
				pre = task.get_preconditions(a)
				var = self.generatePairActionControllerState(a, n)
				for p in pre:
					varPre = self.generateAtomControllerState(p, n)
					self.addCls(['-' + var, varPre])
			...
			for act in acts:
				...
				for act2 in acts:
					...
					if act2 in other_acts:
						self.addCls(['-' + var_pair, var_pair2]) # 1
					else:
						self.addCls(['-' + var_pair, '-' + var_pair2]) # 2
	...
	def generateNegativeForwardPropagation(self, task, controllerStates, debug):
		...
		for n1 in controllerStates[len(controllerStates)-2:(len(controllerStates)-1 if len(controllerStates) > 2 else len(controllerStates))]:
			for n2 in controllerStates:
				for p in atoms:
					...
					for a in task.get_relevant_actions(p):
						...
						if p in del_list:
							self.addCls(['-' + var_triplet, '-' + var_bin,'-' + var_atom_n2]) # 1
						if p in add_list:
							disj_add_clause.append(var_bin)
						if p not in add_list and self.__sibling_action_adds_atom(task, a, p):
							self.addCls(['-' + var_triplet, '-' + var_bin, var_atom_n1, '-' + var_atom_n2]) # 3
					self.addCls(disj_add_clause) # 2

		for n1 in controllerStates:
			for n2 in controllerStates[len(controllerStates)-2:(len(controllerStates)-1 if len(controllerStates) > 2 else len(controllerStates))]:
				for p in atoms:
					var_atom_n1 = self.generateAtomControllerState(p, n1)
					var_pair_n1n2 = self.generatePairCSCS(n1, n2)
					var_atom_n2 = self.generateAtomControllerState(p, n2)
	...
	def generateReachableIinitial(self, initialCState, debug = False):
		c1, v1 = self.get_num_cl_vars()
		start = timer()
		self.addCls([self.generateReachableI(initialCState)])
	...
	def generateReachableGInitial(self, task, goalCState, controllerStates, numberControllerStates, debug = False):
		...
		for i in range(numberControllerStates + 1):
			self.addCls([self.generateReachableG(goalCState, i)])
		# set the negation for non goal states
		for n in controllerStates:
			if n == goalCState:
				continue
			self.addCls(['-' + self.generateReachableG(n, 0)])
		...
		for n in controllerStates:
			...
			for a in actions:
				if '_unfair_' in a:
					...
					self.addCls(['-' + varPair, varPairUnf]) # 1
		...
		for n in controllerStates:
			...
			for a in actions:
				if '_unfair_' not in a:
					varPair = self.generatePairActionControllerState(a, n)
					self.addCls(['-' + varPair, varPairF]) # 2
		...
		for i in range(k):
			for n1 in controllerStates:
				...
				for n2 in controllerStates:
					...
					self.addCls(['-' + varRepl, '-' + varPair, varRG])
					self.addCls(['-' + varRG, varRepl])
					self.addCls([varPair, varRepl])
		...
		for i in range(k):
			for n1 in controllerStates:
				...
				for n2 in controllerStates:
					...
					varRG   = self.generateReachableG(n2, i)
					self.addCls(['-' + varRepl, varPair])
					self.addCls(['-' + varRepl, varFair])
					self.addCls(['-' + varRepl, varRG])
					self.addCls([varRepl, '-' + varPair, '-' + varFair, '-' + varRG])
		# Right arrow
		for n1 in controllerStates:
			if n1 == 'ng':
				continue
		...
		for n1 in controllerStates:
			...
			for i in range(k):
				...
				for n in controllerStates:
					listStrong.append(self.generateReplacementGoalPropagation(n1, n, i))
					listCyclic.append(self.generateReplacementGoalPropagation3(n1, n, i))
				self.addCls([varRG] + ['-' + e for e in listStrong])
