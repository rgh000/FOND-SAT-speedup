...
def clean_and_exit(n1, n2, n3, n4, n5, msg):
	print(msg)
	os.system('rm %s %s %s %s' % (n1, n3, n4, n5))
	exit()
...
for i in range(1000):
	...
	#cnf.reset()
	print('total: %i' % cnf.total)
	print('dups: %i' % cnf.dups)
	start_g = timer()
	ak = cnf.generateAssumption(len(controllerStates))
	cnf.kk = len(controllerStates)
	...
	#command = '../../FOND-SAT-master/src/minisat-old %s %s' % (name_formula_file, name_output_satsolver)
	#command = '/w/32/rhickey/2542/FOND-SAT-master/src/minisat-master/build/release/bin/minisat -mem-lim=%i -cpu-lim=%i %s %s' % (mem_limit, time_for_sat, name_formula_file, name_output_satsolver)
	#command = './cadical-rel-1.4.0/build/cadical -t %i --chrono=0 %s -w %s' % (time_for_sat, name_final, name_output_satsolver)
	print('SAT solver called with %i MB and %i seconds' % (mem_limit, time_for_sat))
	#os.system(command)
	#ms = Cadical(bootstrap_with=cnf.the_formula.clauses)
	#cnf.ms.append_formula(cnf.the_formula.clauses)
	result2 = cnf.ms.solve(assumptions=[-cnf.mapVariableNumber[ak]])
	fz = open(name_output_satsolver, 'w')
	if result2:
		fz.write("SAT\n")
		fz.write(' '.join(str(x) for x in cnf.ms.get_model()) + '\n')
	else:
		fz.write("UNSAT\n")
	fz.close()
	cnf.ms.add_clause([cnf.mapVariableNumber[ak]])
	end_solv = timer()
